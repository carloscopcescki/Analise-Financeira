...
if selected_indice == "":
    st.warning("Selecione o indice para analisar o rendimento")
else:
...
for ativo, df in dados_ativos.items():
    ...
    if len(df) > 1:
        ...
        if rendimento_total < 0:
            st.write(f"**Rendimento:** <span style='color:{color_negative}'>{rendimento_total:.2%}</span>", unsafe_allow_html=True)
        else:
            st.write(f"**Rendimento:** <span style='color:{color_positive}'>{rendimento_total:.2%}</span>", unsafe_allow_html=True)
    else:
